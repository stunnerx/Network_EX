...
# 3. 创建实验保存目录
# =========================
def make_run_dir(base_dir, run_name=None, batch_size=128, lr=1e-3, num_epochs=30, seed=42):
    os.makedirs(base_dir, exist_ok=True)

    if run_name is None or run_name.strip() == "":
        time_str = datetime.now().strftime("%Y%m%d_%H%M%S")
        run_name = f"ResNet18_bs{batch_size}_lr{lr}_ep{num_epochs}_seed{seed}_{time_str}"
...
class ResNet18SVHN(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # 针对 32x32 输入做修改
        # 原始 resnet18 的 conv1 是 7x7, stride=2
        # 这里改成 3x3, stride=1，更适合小尺寸图像
...
def main():
    ...
    # 手动命名文件夹；设为 None 就自动命名
    # run_name = "SVHN_ResNet18"
    run_name = None
